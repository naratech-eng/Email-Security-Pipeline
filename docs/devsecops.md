...
**Gap 3 — dependencies unpinned: CLOSED (surfaced independently by SonarCloud, rules `S8541`/`S8544`/`S6505`/`S6506`, ~10 findings across the Dockerfile and several workflows — the same gap this section had already named, confirmed by a second tool).** `backend/requirements.txt` now pins every direct dependency to `==`, resolved via `pip-compile` against the previous `>=` floors. Deliberately **not** a full transitive lock (~75 packages) — direct pins only, simpler to maintain and compatible with how Dependabot's pip ecosystem operates.

One thing genuinely worth knowing, stated in the file itself: the pins are **today's newest-compatible resolution, not the old floors rolled forward** — `numpy>=1.26` has no wheels for Python 3.13 (this project's runtime), so the unpinned installs were already silently resolving to a modern release on every fresh build. Pinning today's resolution doesn't add new version risk over what was already running; it stops *future* silent drift. The real residual risk is model compatibility: the M6 `joblib`/TensorFlow artifacts were serialized against whatever versions were installed at training time, quite possibly older than `pandas==3.0.5`/`scikit-learn==1.9.0`/`tensorflow==2.21.0` now pinned. Verified clean for import and the 9 local unit tests; **not** verified against the real S3 models locally (no AWS access in the environment this was fixed from) — `backend-tests.yml`'s integration tests load the actual models and are the real gate here. Also pinned: the ad-hoc CI-only tool installs (`bandit`, `pip-audit`, `semgrep`, `schemathesis`, `pytest`, `pytest-cov`) across `sast.yml`/`sonarcloud.yml`/`dast-nightly.yml`/`backend-tests.yml`. **Caveat:** Dependabot's pip ecosystem watches `requirements.txt` files, not inline `pip install x==y` strings inside workflow scripts — these six will go stale without someone noticing unless bumped manually or moved into a real requirements file later.

Also closed in the same pass: **~20 GitHub Actions pinned to commit SHA** (`S7637`) instead of a mutable version tag, across every workflow — a tag can be force-moved to point at different code without your knowledge; a SHA can't. Comments preserve the human-readable tag (`@abc123... # v6`) so `git blame`/review stays legible. Dependabot's `github-actions` ecosystem (`.github/dependabot.yml`) understands this convention and bumps both the SHA and the comment together, so this doesn't trade automatic updates for security — the entry was added specifically because of this pattern. And **`S8482` (blocker)** — `sast.yml`'s actionlint install used `curl | bash` with no verification of the downloaded script; replaced with a direct release-tarball download checked against a pinned `sha256`, verified locally that a tampered/wrong checksum is correctly rejected (`sha256sum -c` exits 1).
