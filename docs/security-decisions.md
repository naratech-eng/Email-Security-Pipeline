...
| Dependabot PRs need secrets copied to the Dependabot store | Dependabot PRs run restricted and can't read normal Actions secrets, so `backend-tests.yml`/`terraform-pr.yml` fail on them until `AWS_OIDC_ROLE_ARN` and the `TF_VAR_*` secrets are added under Settings → Secrets → **Dependabot** | `.github/dependabot.yml` |
