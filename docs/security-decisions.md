...
| Pre-commit hooks are opt-in, not enforceable | `.pre-commit-config.yaml` exists (SEC-PRECOMMIT), but a repo cannot force hooks on a clone — each developer must run `pre-commit install`, and any hook is bypassable with `--no-verify`. The CI gitleaks job remains the enforcement backstop | `docs/devsecops.md` §3.5 |
| Dependabot PRs need secrets copied to the Dependabot store | Dependabot PRs run restricted and can't read normal Actions secrets, so `backend-tests.yml`/`terraform-pr.yml` fail on them until `AWS_OIDC_ROLE_ARN`, `TF_VAR_DB_PASSWORD`, and `TF_VAR_KEY_NAME` are added under Settings → Secrets → **Dependabot** (a separate scope, applied once — not per-branch). `TF_VAR_AMPLIFY_GITHUB_ACCESS_TOKEN` is *not* required: it's optional by design and only consumed when the Amplify app is first created | `.github/dependabot.yml` |
...
| Pinned ad-hoc CI tool versions (bandit, pip-audit, semgrep, schemathesis, pytest, pytest-cov) aren't Dependabot-tracked | Dependabot's pip ecosystem watches requirement files, not inline `pip install x==y` in workflow scripts — these six will go stale silently unless bumped manually | `.github/workflows/sast.yml`, `sonarcloud.yml`, `dast-nightly.yml`, `backend-tests.yml` |
...
- **Pre-commit hooks (SEC-PRECOMMIT).** §3.5's highest-value shift-left gap. `.pre-commit-config.yaml` runs gitleaks (sharing `.gitleaks.toml` with CI), `detect-private-key`, a 5 MB large-file guard, YAML/JSON/TOML validation, and `terraform fmt` before a commit is created — the one layer that *prevents* a secret leak rather than reporting one after it's already public. Remains opt-in per clone and bypassable, so it's tracked above as a partial rather than an absolute control.
...
- **SonarCloud's first real scan, and the fixes it drove.** 191 findings, mostly noise (public IP on the mail EC2, RDS backup retention — both already-accepted design decisions Sonar just doesn't know about) but three real ones fixed: the `curl | bash` actionlint install (blocker — replaced with a checksum-verified release download), ~20 GitHub Actions pinned tag→commit-SHA across every workflow, and `backend/requirements.txt` pinned to exact versions (closing "Gap 3" above). Also caught and fixed a real vulnerability in the scanner action itself (`SonarSource/sonarqube-scan-action@v5` had a known CVE; bumped to `@v6`) and a missing `workflow_dispatch` trigger that meant there was no way to re-run the workflow on demand after the token was added mid-run.
